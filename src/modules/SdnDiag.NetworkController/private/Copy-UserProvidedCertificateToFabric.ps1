    ...
    if ($RotateNodeCerts) {
        foreach ($controller in $FabricDetails.NetworkController) {
            ...
            if ($null -ne $nodeCertConfig.Cert.FileInfo.FullName) {
                ...
                "Unable to locate self-signed certificate file for {0}. Node certificate may need to be manually installed to other Network Controllers manually." -f $controller | Trace-Output -Level:Failure
            }
        }
    }
