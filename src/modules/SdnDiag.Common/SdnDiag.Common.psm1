...
Import-Module "$PSScriptRoot\SdnDiag.Common.Utilities.psm1"
...
function Start-SdnNetshTrace {
    ...
    param (
        ...
        [Parameter(Mandatory = $true, Position = 1, ParameterSetName = 'Remote')]
        [SdnRoles]$Role,
        ...
        [Parameter(Mandatory = $false, Position = 7, ParameterSetName = 'Local')]
        [Parameter(Mandatory = $false, Position = 7, ParameterSetName = 'Remote')]
        [ValidateSet("Default", "Optional", "All")]
    ...
    try {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'Remote') {
            Invoke-PSRemoteCommand -ComputerName $ComputerName -Credential $Credential -ScriptBlock {
                Start-SdnNetshTrace
            } -ArgumentList $Role, $OutputDirectory.FullName, $Capture, $Overwrite, $Report, $MaxTraceSize, $Providers
        }
        else {
            Start-NetshTrace -OutputDirectory $OutputDirectory.FullName `
            -Capture $Capture -Overwrite $Overwrite -Report $Report -MaxTraceSize $MaxTraceSize, -TraceProviderString $traceProviderString
        }
    }
    catch {
