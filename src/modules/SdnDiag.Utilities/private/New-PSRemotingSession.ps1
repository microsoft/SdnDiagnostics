function New-PSRemotingSession {
    ...
    process {
        $ComputerName | ForEach-Object {
            ...
            if ($currentActiveSessions.ComputerName -contains $_ -and !$Force) {
                $session = ($currentActiveSessions | Where-Object { $_.ComputerName -eq $_ })[0]
                "Located existing powershell session {0} for {1}" -f $session.Name, $_ | Trace-Output -Level:Verbose
            ...
            $isIpAddress = ($_ -as [IPAddress]) -as [Bool]
            if ($isIpAddress) {
                try {
                    Confirm-IsAdmin

                    "{0} is an ip address" -f $_ | Trace-Output -Level:Verbose
                    $trustedHosts = Get-Item -Path "WSMan:\localhost\client\TrustedHosts"
                    if ($trustedHosts.Value -notlike "*$_*" -and $trustedHosts.Value -ne "*") {
                        "Adding {0} to {1}" -f $_, $trustedHosts.PSPath | Trace-Output
                        Set-Item -Path "WSMan:\localhost\client\TrustedHosts" -Value $_ -Concatenate
                    }
                }
                catch {
            ...
            try {
                if ($Credential -ne [System.Management.Automation.PSCredential]::Empty) {
                    ...
                    $session = New-PSSession -Name "SdnDiag-$(Get-Random)" -ComputerName $_ -Credential $Credential -SessionOption (New-PSSessionOption -Culture en-US -UICulture en-US -IdleTimeout 86400000) -ErrorAction Stop
                }
                else {
                    # if the credential is not defined, we want to check if we
                    if ($PSSenderInfo -and !(Get-WSManCredSSPState)) {
                        throw New-Object System.NotSupportedException("Unable to create PSSession to $_. This operation is not supported in a remote session without supplying -Credential.")
                    }
                    ...
                    if ($isIpAddress -and $Credential -eq [System.Management.Automation.PSCredential]::Empty) {
                        throw New-Object System.NotSupportedException("Unable to create PSSession to $_. The Credential parameter is required when using an IP Address.")
                    }

                    "PSRemotingSession use default credential" | Trace-Output -Level:Verbose
                    $session = New-PSSession -Name "SdnDiag-$(Get-Random)" -ComputerName $_ -SessionOption (New-PSSessionOption -Culture 'en-US' -UICulture 'en-US' -IdleTimeout 86400000) -ErrorAction Stop
                }

                "Created powershell session {0} to {1}" -f $session.Name, $_ | Trace-Output -Level:Verbose
                if ($ImportModuleOnRemoteSession) {
                    "Importing module {0} on remote session {1}" -f $ModuleName, $session.Name | Trace-Output -Level:Verbose
                    Invoke-Command -Session $session -ScriptBlock $importRemoteModule -ArgumentList @($ModuleName, $Global:SdnDiagnostics.Config) -ErrorAction Stop
                }
                ...
                "Unable to create powershell session to {0}`n`t{1}" -f $_, $_.Exception.Message | Trace-Output -Level:Error
                continue
            }
        }
    }
