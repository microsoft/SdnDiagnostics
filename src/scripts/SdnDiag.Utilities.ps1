...
function Invoke-PSRemoteCommand {
    ...
    if ($session) {
        ...
        if ($AsJob) {
            $result = Invoke-Command -Session $session -ScriptBlock $ScriptBlock -AsJob -JobName $([guid]::NewGuid().Guid) -ArgumentList $ArgumentList -ErrorAction Stop
            if ($PassThru) {
                if ($Activity) {
...
function Initialize-DataCollection {
    ...
    if ($Configuration) {
        $confirmFeatures = Confirm-RequiredFeaturesInstalled -Name $Configuration.WindowsFeature
        if (-NOT ($confirmFeatures)) {
            "Required feature is missing: {0}" -f ($Configuration.WindowsFeature -join ', ') | Trace-Output -Level:Exception
            return $false
        }

        $confirmModules = Confirm-RequiredModulesLoaded -Name $Configuration.RequiredModules
        if (-NOT ($confirmModules)) {
            "Required module is not loaded: {0}" -f ($Configuration.RequiredModules -join ', ') | Trace-Output -Level:Exception
            return $false
        }
    }
